...
from flask_jwt_extended import (
    ...
    jwt_required, get_jwt_identity, get_jwt, jwt
)
...
# JWT access token lifetime in seconds (must match flask_jwt_extended config)
JWT_ACCESS_TOKEN_TTL = 8 * 3600  # 8 hours
...
def check_if_token_revoked(jwt_header, jwt_payload: dict) -> bool:
    ...
    Uses cache_manager (Redis-backed with memory fallback) for persistence.
    """
    jti = jwt_payload.get("jti")
    if not jti:
        return False
    # Check if JTI exists in blacklist cache
    return cache_manager.exists(f"jwt_blacklist:{jti}")
...
def logout():
    ...
    try:
        jti = get_jwt()["jti"]
        # Store blacklisted JTI in cache with TTL = token remaining lifetime
        # Entry auto-expires when the token would have expired anyway
        cache_manager.set(f"jwt_blacklist:{jti}", True, ttl=JWT_ACCESS_TOKEN_TTL)
