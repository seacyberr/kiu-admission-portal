...
DB_CONNECTIONS = Gauge('kiu_db_connections_active', 'Active database connections')
...
ACTIVE_USERS = Gauge('kiu_active_users', 'Number of active users (authenticated in last hour)')

APPLICATIONS_TOTAL = Gauge('kiu_applications_total', 'Total admission applications', ['status'])
...
OPPORTUNITIES_TOTAL = Gauge('kiu_opportunities_total', 'Total active opportunities', ['type'])

CAREER_PATHS_TOTAL = Gauge('kiu_career_paths_total', 'Total career paths')
...
def update_application_metrics():
    ...
    # BUG FIX: import func inside the function where it's used
...
def update_opportunity_metrics():
    ...
    BUG FIX: 'func' was used but never imported in this function scope,
    causing a NameError crash whenever this function was called (at startup
    and on every /api/metrics scrape).
    """
    # BUG FIX: added the missing imports
...
def init_metrics(app):
    ...
    with app.app_context():
        try:
            ...
            pass  # Tables may not exist yet at first boot
