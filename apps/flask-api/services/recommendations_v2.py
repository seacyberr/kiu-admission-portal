...
def assess_qualifications():
    ...
    try:
        ...
        # Simplified assessment (recommendation engine removed)
        # Return basic eligibility based on level
        result = {
            "qualification_type": highest_education,
            "eligible": True,
            "recommendations": [
                {"program": "Bachelor of Science", "match": 85},
                {"program": "Bachelor of Arts", "match": 75}
            ],
            "pathway": "direct",
            "warnings": []
        }
...
def compare_programs():
    ...
    try:
        ...
        # Simplified comparison (recommendation engine removed)
        return success_response({"programs": program_codes, "comparison": "simplified"})
...
def list_programs():
    ...
    programs = query.all()
    filtered = [p.to_dict() for p in programs]
...
def get_program_details(program_code):
    ...
    # Find by name or id
    program = Program.query.filter(Program.name.ilike(f"%{program_code}%")).first()
    if not program:
